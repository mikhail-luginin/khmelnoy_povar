...
def update():
    ...
    for storage in storages_xml.findall('corporateItemDto'):
        ...
        try:
            storage_row = Storage.objects.get(storage_id=storage_id)
        except Storage.DoesNotExist:
            storage_row = Storage(storage_id=storage_id, name=name,
...
def storages_all() -> List[Storage]:
    return Storage.objects.filter(is_office=0)


def storage_get(**kwargs) -> Storage | None:
    return Storage.objects.filter(**kwargs).first()
